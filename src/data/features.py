...
class FeatureEngineer:
    ...
    def _create_financial_ratios(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'annual_inc' in df.columns and 'loan_amnt' in df.columns:
            df['loan_income_ratio'] = df['loan_amnt'] / (df['annual_inc'] + 1e-8)  # Add small value to avoid division by zero
        
        # Installment to income ratio
        if 'annual_inc' in df.columns and 'installment' in df.columns:
            df['installment_income_ratio'] = (df['installment'] * 12) / (df['annual_inc'] + 1e-8)
    ...
    def _create_categorical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'purpose' in df.columns:
            ...
            df['is_debt_consolidation'] = df['purpose'].isin(debt_consolidation_purposes).astype(int)
    ...
    def _create_risk_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'dti' in df.columns:
            df['high_dti_risk'] = (df['dti'] > 25).astype(int)  # DTI > 25% is considered high risk
        
        if 'int_rate' in df.columns:
            df['high_interest_risk'] = (df['int_rate'] > 15).astype(int)  # Interest rate > 15% is high risk
        
        if 'revol_util' in df.columns:
            df['high_revol_util_risk'] = (df['revol_util'] > 80).astype(int)  # Revolving utilization > 80% is high risk
    ...
    def create_target_variable(self, df: pd.DataFrame, target_col: str = 'default') -> pd.DataFrame:
        """
        ...
        if 'loan_status' in df.columns:
            ...
            df[target_col] = df['loan_status'].map(default_mapping).fillna(0).astype(int)
