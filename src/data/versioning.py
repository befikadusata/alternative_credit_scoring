...
class DataVersioner:
    ...
    def save_dataset_version(
        ...
        self.logger.info(f"Dataset version saved: {dataset_name} with hash {data_hash[:8]} in run {run.info.run_id}")
        return run.info.run_id
    
    def load_dataset_version(self, run_id: str, dataset_name: str, artifact_path: str = "datasets") -> pd.DataFrame:
        """
        ...
        if not dataset_files:
            raise FileNotFoundError(f"No dataset file found for {dataset_name} in run {run_id}")
    ...
    def register_reference_dataset(
        ...
        self.logger.info(f"Reference dataset registered: {dataset_name} in run {ref_run_id}")
        return ref_run_id


def create_reference_dataset(
    raw_data_path: str,
    output_path: str = "data/reference/reference_dataset.csv"
) -> pd.DataFrame:
    """
    ...
    if 'default' not in df_engineered.columns and 'loan_status' in df_engineered.columns:
