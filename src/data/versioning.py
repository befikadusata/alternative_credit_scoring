...
from typing import Dict, Optional, Any # Added Any back for stats: Dict[str, Any]
...
class DataVersioner:
    ...
    def __init__(self, mlflow_tracking_uri: Optional[str] = None):
        ...
        except Exception as e:  # Fix E722: do not use bare 'except'
            self.logger.warning(
                f"MLflow experiment '{self.experiment_name}' already exists or failed to "
                f"create: {e}. Attempting to get existing experiment."
            )
    ...
    def _get_data_statistics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        stats: Dict[str, Any] = {  # Add type hint for stats dictionary
            "row_count": len(df),
    ...
    def save_dataset_version(
        ...
        self.logger.info(
            f"Dataset version saved: {dataset_name} with hash {data_hash[:8]} in run "
            f"{run.info.run_id}"  # Fix E501
        )
    ...
    def load_dataset_version(
        ...
        Returns:
            Loaded DataFrame
        """
        # Fix F841: local variable 'artifact_uri' is assigned to but never used
        # The mlflow.get_artifact_uri call is not directly needed if we use download_artifacts
        # and then glob for the file.
        
        # Download the artifact
        local_path = mlflow.artifacts.download_artifacts(
            run_id=run_id, artifact_path=artifact_path  # Fix E501
        )
