...
def run_drift_analysis(reference_data_path: str, current_data_path: str, output_report_path: str, prometheus_url: str):
    """
    ...
    report = Report(metrics=[
        DataDriftPreset(),
        ClassificationPreset(prediction_type='binary', target_name='target', prediction_name='prediction')
    ])
    ...
    try:
        ...
        drift_score = report_dict['metrics'][0]['result']['data_drift']['data']['metrics']['dataset_drift_score']
        num_drifted_features = report_dict['metrics'][0]['result']['data_drift']['data']['metrics']['n_drifted_columns']
        
        registry = CollectorRegistry()
        g_drift_score = Gauge('data_drift_score', 'Overall data drift score from Evidently', registry=registry)
        g_num_drifted_features = Gauge('num_drifted_features', 'Number of features that have drifted', registry=registry)

        g_drift_score.set(drift_score)
        g_num_drifted_features.set(num_drifted_features)

        if prometheus_url:
            push_to_gateway(prometheus_url, job='drift-analysis-batch', registry=registry)
            logger.info(f"Successfully pushed metrics to Prometheus Pushgateway at: {prometheus_url}")
        else:
            logger.warning("Prometheus Pushgateway URL not provided. Skipping metric push.")

    except KeyError as e:
        logger.error(f"Could not extract metric from Evidently report: {e}. Report structure might have changed.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="reports/drift_report.html", help="Path to save the HTML drift report.")
    parser.add_argument("--prometheus-url", default=os.getenv("PROMETHEUS_PUSHGATEWAY_URL"), help="URL of the Prometheus Pushgateway.")
