...
def load_model_from_registry(
    ...
    Args:
        ...
        model_version: Version of the model (can be version number, "latest",
                       or alias like "champion")
    ...
    logger.info(
        f"Attempting to load model '{model_name}' version/alias '{model_version}' "
        "from registry"
    )
    ...
    try:
        ...
        if model_version == "latest":
            ...
            for version_info in latest_version_info:
                if version_info.current_stage == "Production":
                    ...
                    version_info.current_stage == "Staging" and latest_version is None
                ):
                    latest_version = version_info.version
                    break
            ...
            logger.info(
                f"Resolved 'latest' to version {latest_version}, URI: {model_uri}"
            )
        else:
            ...
            try:
                ...
                alias_version_info = client.get_model_version_by_alias(
                    model_name, str(model_version)
                )
                model_uri = f"models:/{model_name}/{alias_version_info.version}"
                logger.info(
                    f"Resolved alias '{model_version}' to version "
                    f"{alias_version_info.version}, URI: {model_uri}"
                )
            except Exception:  # noqa: E722 - flake8 wants specific exception
                # Assume it's a version number
                model_uri = f"models:/{model_name}/{model_version}"
                logger.info(
                    f"Using direct version '{model_version}', URI: {model_uri}"
                )
        ...
        logger.error(
            f"Failed to load model '{model_name}' version '{model_version}': {str(e)}"
        )
        raise


def get_model_version_info(
    model_name: str, model_version: Union[str, int], tracking_uri: Optional[str] = None
) -> dict:
    """
    ...
    try:
        ...
        logger.error(
            f"Failed to get model version info for '{model_name}' version "
            f"'{model_version}': {str(e)}"
        )
        raise
...
def validate_model_features(model: mlflow.pyfunc.PyFuncModel, input_data) -> bool:
    ...
    try:
        ...
        model.predict(input_data)  # Removed assignment to unused 'prediction'
        return True
    except Exception as e:
