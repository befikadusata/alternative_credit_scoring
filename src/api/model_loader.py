...
def load_model_from_registry(
    ...
    try:
        ...
        if os.path.exists(cleaner_path):
            ...
            logger.warning(f"DataCleaner not found at {cleaner_path}. Returning None for cleaner.")
            data_cleaner = None
        ...
        if os.path.exists(feature_names_path):
            ...
            logger.warning(f"Feature names not found at {feature_names_path}. Generating default feature names.")
            ...
            raw_input_data = {
                ...
                'home_ownership': ['MORTGAGE'],  # Possible values: MORTGAGE, RENT, OWN, OTHER, NONE, ANY
                'verification_status': ['Verified'],  # Possible values: Verified, Not Verified, Source Verified
                'purpose': ['debt_consolidation'],  # Possible values: debt_consolidation, credit_card, home_improvement, etc.
            ...
            all_sub_grades = [f"{g}{i}" for g in ['A', 'B', 'C', 'D', 'E', 'F', 'G'] for i in range(1, 6)]
            all_home_ownership = ['MORTGAGE', 'RENT', 'OWN', 'OTHER', 'NONE', 'ANY']
            all_verification_status = ['Verified', 'Not Verified', 'Source Verified']
            all_purposes = ['debt_consolidation', 'credit_card', 'home_improvement', 'other', 'major_purchase', 'medical', 'car', 'small_business', 'wedding', 'house', 'moving', 'vacation']
            ...
            X_raw = pd.DataFrame({
                ...
                'term': (all_terms * (len(all_grades) // len(all_terms) + 1))[:len(all_grades)],
                'grade': all_grades,
                'sub_grade': all_sub_grades[:len(all_grades)],  # Use a subset that matches grade length
                'home_ownership': (all_home_ownership * (len(all_grades) // len(all_home_ownership) + 1))[:len(all_grades)],
                'verification_status': (all_verification_status * (len(all_grades) // len(all_verification_status) + 1))[:len(all_grades)],
                'purpose': (all_purposes * (len(all_grades) // len(all_purposes) + 1))[:len(all_grades)],
                'initial_list_status': (all_initial_list_status * (len(all_grades) // len(all_initial_list_status) + 1))[:len(all_grades)],
            })
            ...
            logger.info(f"Generated default feature names with {len(feature_names)} features.")

        # Return a dummy model and the loaded cleaner
        model = DummyModel()
        logger.info(f"Returning DummyModel for '{model_name}' version '{model_version}'")
