...
def validate_model_performance(run_id: str, min_auc_threshold: float = 0.7, mlflow_tracking_uri: str = "http://localhost:5000"):
    """
    ...
    if 'auc' in metrics:
        if metrics['auc'] < min_auc_threshold:
            logger.error(f"AUC {metrics['auc']} is below minimum threshold {min_auc_threshold}")
            return False
        else:
            logger.info(f"AUC {metrics['auc']} passes minimum threshold {min_auc_threshold}")
    else:
...
def validate_fairness_report(fairness_report_path: str, max_disparity_threshold: float = 0.1):
    """
        ...
        if has_bias:
            ...
            for metric, value in disparities.items():
                if value > max_disparity_threshold:
                    logger.error(f"Disparity for {metric} ({value}) exceeds threshold ({max_disparity_threshold})")
                    return False
...
def register_model(
    ...
    if fairness_report_path and not validate_fairness_report(fairness_report_path, max_disparity_threshold):
    ...
    try:
        ...
        if stage.lower() in ["staging", "production"]:
            ...
            logger.info(f"Model version {result.version} transitioned to {stage.capitalize()}")
        ...
        logger.info(f"Model {model_name} version {result.version} successfully registered and validated")
...
def main(
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        if version:
            logger.info(f"Model registration completed successfully. Registered model: {model_name}, version: {version}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Register the final, validated model in MLflow Model Registry.")
    parser.add_argument(
        "--run_id",
        type=str,
        required=True,
    ...
    try:
        ...
        if version:
            logger.info(f"Model registration completed successfully! Model: {args.model_name}, Version: {version}")
        else:
            logger.error("Model registration failed.")
            sys.exit(1)
