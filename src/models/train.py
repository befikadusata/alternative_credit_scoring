...
def main(
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        X_val[numeric_cols] = X_val[numeric_cols].fillna(X_train[numeric_cols].median())  # Use training median
        ...
        if model_type in ['logistic_regression', 'random_forest']:
            mlflow.sklearn.log_model(
                sk_model=model,
                artifact_path="model",
                conda_env="environment.yml" if os.path.exists("environment.yml") else None,
        ...
        logger.info(f"Model training completed. Run ID: {mlflow.active_run().info.run_id}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train a baseline model for credit scoring.")
    parser.add_argument(
        "--train_data_path",
        type=str,
