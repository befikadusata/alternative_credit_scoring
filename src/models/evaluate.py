...
class ModelEvaluator:
    ...
    def evaluate_model(self, model, X_test, y_test, model_name="Model", save_plots=True):
        ...
        self.logger.info(f"{model_name} - Accuracy: {results['accuracy']:.4f}, "
                        ...
                        if isinstance(metrics, dict):
                            for sub_metric, value in metrics.items():
                                if isinstance(value, (int, float)):
                                    mlflow.log_metric(f"{sub_metric}_{class_name}", value)
        ...
        if save_plots and y_pred_proba is not None:
            self._create_visualizations(X_test, y_test, y_pred, y_pred_proba, model_name)
    ...
    def _create_visualizations(self, X_test, y_test, y_pred, y_pred_proba, model_name):
        ...
        axes[0, 1].plot(recall_vals, precision_vals, label=f'PR Curve (AP = {avg_precision:.3f})', color='red')
    ...
    def create_shap_explanation(self, model, X_test, model_name="Model"):
        ...
        try:
            ...
            if hasattr(model, 'booster'):
                ...
                explainer = shap.LinearExplainer(model, X_test[:100])  # Use subset for efficiency
                shap_values = explainer.shap_values(X_test)
    ...
    def evaluate_model_by_group(self, model, X_test, y_test, sensitive_features, model_name="Model"):
        """
        ...
        for col in sensitive_features.columns:
            ...
            for value in unique_values:
                ...
                group_metrics[value] = {
                    ...
                    'accuracy': accuracy_score(y_test_group, y_pred_group) if len(y_test_group) > 0 else 0,
                    'precision': precision_score(y_test_group, y_pred_group, zero_division=0) if len(y_test_group) > 0 else 0,
                    'recall': recall_score(y_test_group, y_pred_group, zero_division=0) if len(y_test_group) > 0 else 0,
                    'f1_score': f1_score(y_test_group, y_pred_group, zero_division=0) if len(y_test_group) > 0 else 0,
                }
                
                if y_pred_proba is not None and len(y_test_group) > 0:
                    group_metrics[value]['auc'] = roc_auc_score(y_test_group, y_pred_proba_group)
                else:
            ...
            for feature, metrics in results_by_group.items():
                for group, group_metrics in metrics.items():
                    for metric_name, metric_value in group_metrics.items():
                        if isinstance(metric_value, (int, float)):
                            mlflow.log_metric(f"{feature}_{group}_{metric_name}", metric_value)
        except:
            # If not in MLflow run, continue without logging
            pass
...
def main(
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        logger.info(f"Model evaluation completed. Run ID: {mlflow.active_run().info.run_id}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate a trained credit scoring model.")
    parser.add_argument(
        "--test_data_path",
        type=str,
