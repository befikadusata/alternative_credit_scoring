...
class FairnessAnalyzer:
        ...
        for col in sensitive_features.columns:
            ...
            for value in unique_values:
                ...
                group_metrics[value] = {
                    ...
                    'true_positive_rate': ((y_pred_group == 1) & (y_true_group == 1)).sum() / (y_true_group == 1).sum() if (y_true_group == 1).sum() > 0 else 0,  # Equal opportunity
                    'false_positive_rate': ((y_pred_group == 1) & (y_true_group == 0)).sum() / (y_true_group == 0).sum() if (y_true_group == 0).sum() > 0 else 0,
                ...
                if len(np.unique(y_true_group)) > 1:
                    try:
                        group_metrics[value]['auc'] = roc_auc_score(y_true_group, y_pred_group)
                    except:
                        group_metrics[value]['auc'] = -1  # Placeholder
                else:
                    group_metrics[value]['auc'] = -1  # Placeholder for groups with only one class
    ...
    def _calculate_disparities(self, group_metrics):
        ...
        for metric in metrics_of_interest:
            values = [gm[metric] for gm in group_metrics.values() if metric in gm and gm[metric] != -1]
            if len(values) > 1:
                ...
                disparities[f'{metric}_cv'] = np.std(values) / np.mean(values) if np.mean(values) != 0 else 0
        ...
        for feature, groups in fairness_results.items():
            if 'disparities' in groups:
                ...
                significant_disparities = {
                    metric: value for metric, value in disparities.items() 
                    if value > threshold
                }
                ...
                if significant_disparities:
                    bias_indicators[feature] = {
                        ...
                        'concern': 'Potential bias detected' if significant_disparities else 'No significant bias detected'
                    }
                else:
                    bias_indicators[feature] = {
                        'disparities': {},
    ...
    def create_fairness_report(self, fairness_results, bias_indicators):
        ...
        for feature, groups in fairness_results.items():
            ...
            if len(groups) > 0 and 'disparities' in groups:
                ...
                sample_group = {k: v for k, v in next(iter(groups.values())).items() 
                               if k != 'disparities' and not k.startswith('count')}
                
                for metric in sample_group.keys():
                    values = [g[metric] for g in groups.values() 
                             if metric in g and g[metric] != -1 and 'disparities' not in g]  # Exclude disparities and groups with placeholder values
                    if values:
                        summary[metric] = {
                            'min': min(values),
                            'max': max(values),
    ...
    def create_fairness_visualizations(self, fairness_results, model_name="Model", save_plots=True):
        """
        ...
        for feature, groups in fairness_results.items():
            ...
            if 'disparities' in groups:
                groups_no_disparities = {k: v for k, v in groups.items() if k != 'disparities'}
            else:
            ...
            metrics_to_plot = ['positive_rate', 'true_positive_rate', 'false_positive_rate', 'accuracy']
            
            # Remove metrics not present in the data
            available_metrics = [m for m in metrics_to_plot if any(m in g for g in groups_no_disparities.values())]
...
def analyze_model_fairness(model, X_test, y_test, sensitive_features, model_name="Model"):
    """
    ...
    fairness_results = analyzer.calculate_fairness_metrics(y_test, y_pred, sensitive_features)
    ...
    try:
        ...
        for feature, bias_info in report['bias_indicators'].items():
            ...
            if 'disparities' in bias_info:
                max_disparity = max([v for v in bias_info['disparities'].values()], default=0)
...
def main(
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        sensitive_features = load_sensitive_features(test_data_path, sensitive_feature_cols)
        ...
        logger.info(f"Fairness analysis completed. Run ID: {mlflow.active_run().info.run_id}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Perform fairness analysis on a trained credit scoring model.")
    parser.add_argument(
        "--test_data_path",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=['home_ownership', 'purpose'],  # Common sensitive features in credit data
        help="List of sensitive feature column names (default: ['home_ownership', 'purpose'])"
    )
    parser.add_argument(
        "--run_id",
        type=str,
        help="MLflow run ID (optional if using --model_name)"
    )
    parser.add_argument(
        "--model_name",
        type=str,
