...
def main(raw_data_path: str, processed_data_path: str, test_size: float = 0.2, random_state: int = 42):
    """
    ...
    logger.info(f"Feature engineering completed. Shape after feature engineering: {df_engineered.shape}")
    ...
    if 'default' in df_engineered.columns:
        ...
        X = df_engineered
        y = None
        logger.warning("No target column 'default' found in the data. All columns will be treated as features.")
    ...
    if y is not None:
        ...
        logger.info(f"Feature-only split - Train: {X_train.shape}, Test: {X_test.shape}")
